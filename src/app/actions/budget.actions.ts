...
import { CreateBudgetFormSchemaType, CreateBudgetFormSchema } from "@/app/schema/budget.schema";
import { requireUser } from "@/app/utils/auth.utils";

type ServerBudgetData = Omit<CreateBudgetFormSchemaType, 'startDate' | 'endDate'> & {
    startDate: Date;
    endDate: Date;
};
...
const createBudget = async (data: ServerBudgetData): Promise<CreateBudgetResponse> => {
    const user = await requireUser();
    const { budgetCategory, startDate, endDate, amount, currency, description, periodType } = data;
...
export { createBudget, type CreateBudgetResponse, getBudgets };
